...
Write-Host "2. Starting Gradle AAB (App Bundle) Build..." -ForegroundColor Cyan
cd android
$env:JAVA_HOME="D:\Android\Android Studio\jbr"
$keystoreAbsPath = Join-Path (Get-Location) "app\my-release-key.keystore"
.\gradlew bundleRelease "-Pandroid.injected.signing.store.file=$keystoreAbsPath" "-Pandroid.injected.signing.store.password=$env:ANDROID_KEYSTORE_PASSWORD" "-Pandroid.injected.signing.key.alias=$env:ANDROID_KEYSTORE_ALIAS" "-Pandroid.injected.signing.key.password=$env:ANDROID_KEY_PASSWORD"
cd ..
