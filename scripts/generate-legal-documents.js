...
const DEFAULT_SOURCE_DIR = path.join(PROJECT_ROOT, "content", "legal");
...
const WEBSITE_LEGAL_DIR = process.env.PLANIT_LEGAL_WEBSITE_DIR
  ? path.resolve(process.env.PLANIT_LEGAL_WEBSITE_DIR)
  : path.resolve(PROJECT_ROOT, "..", "planit-website", "public", "content", "legal");
const SHOULD_SYNC_WEBSITE = process.env.PLANIT_LEGAL_SYNC_WEBSITE !== "0";
...
const DOCUMENTS = [
  ...
  { type: "licenses", file: "licenses.mdx" },
];
...
function parseDocument(documentConfig, language = "en") {
  const sourceFile = path.join(SOURCE_DIR, language === "en" ? "" : language, documentConfig.file);
...
function validateTranslation(original, translated, config) {
  const source = fs.readFileSync(path.join(SOURCE_DIR, config.file), "utf8").replace(/\r\n/g, "\n");
  const hash = createHash("sha256").update(source).digest("hex");
  if (translated.translationSourceHash !== hash) throw new Error("Outdated legal translation: " + config.file);
  for (const key of ["slug", "version", "effectiveDate"]) {
    if (original[key] !== translated[key]) throw new Error("Legal metadata mismatch: " + config.file + " " + key);
  }
  if (original.sections.length !== translated.sections.length) throw new Error("Incomplete legal translation: " + config.file);
  original.sections.forEach((section, index) => {
    const translation = translated.sections[index];
    if (!translation.title || section.kind !== translation.kind || section.blocks.length !== translation.blocks.length) {
      throw new Error("Legal section mismatch: " + config.file + " section " + index);
    }
    section.blocks.forEach((block, blockIndex) => {
      const target = translation.blocks[blockIndex];
      if (block.type !== target.type || (block.cells && block.cells.length !== target.cells?.length)) {
        throw new Error("Legal block mismatch: " + config.file + " section " + index);
      }
      for (const key of ["text", "term"]) {
        if (block[key] && !target[key]?.trim()) throw new Error("Empty legal translation: " + config.file);
      }
      const urls = (JSON.stringify(block).match(/https?:\/\/[^\s"<>)]*/g) || []).sort();
      const translatedUrls = (JSON.stringify(target).match(/https?:\/\/[^\s"<>)]*/g) || []).sort();
      if (JSON.stringify(urls) !== JSON.stringify(translatedUrls)) throw new Error("Legal links mismatch: " + config.file + " section " + index + " block " + blockIndex);
    });
  });
}

function buildGeneratedSource(documents, ukrainianDocuments) {
...
export const LEGAL_DOCUMENTS_UK = ${JSON.stringify(Object.fromEntries(ukrainianDocuments.map((document) => [document.type, document])), null, 2)};
...
function getDocumentSourceFile(documentConfig) {
  return path.join(SOURCE_DIR, documentConfig.file);
}

function getWebsiteTargetFile(documentConfig) {
  return path.join(WEBSITE_LEGAL_DIR, documentConfig.file);
}

function hasWebsiteLegalDirectory() {
  return fs.existsSync(WEBSITE_LEGAL_DIR);
}

function syncWebsiteDocuments() {
  if (!SHOULD_SYNC_WEBSITE) {
    return false;
  }

  if (!hasWebsiteLegalDirectory()) {
    console.warn(`PlanIt website legal directory not found at ${WEBSITE_LEGAL_DIR}. Skipping website MDX sync.`);
    return false;
  }

  for (const documentConfig of DOCUMENTS) {
    const sourceFile = getDocumentSourceFile(documentConfig);
    const targetFile = getWebsiteTargetFile(documentConfig);
    ensureDirectoryFor(targetFile);
...
function assertWebsiteDocumentsCurrent() {
  if (!SHOULD_SYNC_WEBSITE || !hasWebsiteLegalDirectory()) {
    return;
  }
  ...
  for (const documentConfig of DOCUMENTS) {
    ...
    if (sourceContent !== targetContent) {
      throw new Error(`Website legal MDX is out of date: ${targetFile}. Run npm run legal:sync.`);
    }
  }
}
...
function main() {
  ...
  const documents = DOCUMENTS.map((config) => parseDocument(config));
  const ukrainianDocuments = DOCUMENTS.map((config, index) => {
    const translated = parseDocument(config, "uk");
    validateTranslation(documents[index], translated, config);
    return translated;
  });
  const generatedSource = buildGeneratedSource(documents, ukrainianDocuments);
  ...
  if (checkOnly) {
    ...
    assertWebsiteDocumentsCurrent();
    return;
  }

  ensureDirectoryFor(OUTPUT_FILE);
  fs.writeFileSync(OUTPUT_FILE, generatedSource, "utf8");
  const syncedWebsite = syncWebsiteDocuments();
  console.log(`Generated ${documents.length} legal documents from ${SOURCE_DIR}`);
  if (syncedWebsite) {
    console.log(`Synced legal MDX to ${WEBSITE_LEGAL_DIR}`);
  }
}
