...
// ... (Ваш існуючий компонент InputField залишається без змін)
const InputField = ({ icon, placeholder, secureTextEntry, value, onChangeText, isPasswordButton, setIsPasswordVisible, isPasswordVisible, autoCapitalize="none", colors }) => (
  <View style={[styles.inputContainer, { backgroundColor: colors.backgroundColor2, borderColor: colors.borderColor }]}>
    ...
    <TextInput style={[styles.input, { color: colors.textColor }]} placeholder={placeholder} placeholderTextColor={colors.textColor2} secureTextEntry={secureTextEntry} value={value} onChangeText={onChangeText} autoCapitalize={autoCapitalize} />
    {isPasswordButton && (
...
const SignUp = ({ navigation }) => {
  ...
  if (isLangLoading) return <View style={[styles.loadingWrapper, { backgroundColor: colors.backgroundColor }]}><ActivityIndicator size="large" color={colors.accentColor} /></View>;
  ...
  return (
    <AuthLayout title={t('auth.signup.title', lang)} subtitle={t('auth.signup.subtitle', lang)} showBackButton onBack={() => navigation.goBack()}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          <View style={styles.form}>
            ...
            <InputField icon="mail-outline" placeholder={t('auth.fields.email', lang)} value={email} onChangeText={setEmail} colors={colors} />
            <InputField icon="lock-closed-outline" placeholder={t('auth.fields.password', lang)} secureTextEntry={!isPasswordVisible} value={password} onChangeText={setPassword} isPasswordButton={true} isPasswordVisible={isPasswordVisible} setIsPasswordVisible={setIsPasswordVisible} colors={colors} />

            <TouchableOpacity style={[styles.registerButton, { backgroundColor: colors.accentColor, opacity: isLoading ? 0.7 : 1 }]} onPress={handleSignUp} disabled={isLoading}>
              {isLoading ? <ActivityIndicator color="#fff" /> : <Text style={styles.registerButtonText}>{t('auth.signup.submit', lang)}</Text>}
            </TouchableOpacity>
            ...
            <View style={styles.footer}>
              ...
              <TouchableOpacity onPress={() => navigation.navigate('SignIn')}>
                <Text style={[styles.footerLink, { color: colors.accentColor }]}> {t('auth.signup.login_link', lang)}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  registerButton: { height: 56, borderRadius: 16, alignItems: 'center', justifyContent: 'center', marginTop: 10 },
