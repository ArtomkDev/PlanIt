...
export default function StartDateScreen({ schedule, onDataChange, themeColors }) {
    ...
    return (
        <View style={styles.inputContainer}>
            ...
            {showPicker && (
                <View style={Platform.OS === 'web' ? styles.webPicker : [styles.pickerContainer, { backgroundColor: themeColors.backgroundColor2 }]}>
                    {Platform.OS === 'web' ? (
                        <DatePicker selected={selectedDate} onChange={(date) => handleDateSelection(date)} inline />
                    ) : (
                        <DateTimePicker value={selectedDate} mode="date" display="default" onChange={(event, date) => handleDateSelection(date)} />
                    )}
                </View>
            )}
        </View>
